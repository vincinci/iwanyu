  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.results.failed > 0) {
        setError(`${response.results.failed} products failed to import. See errors below.`);
      }
    } catch (err: any) {
      setError(err.message || 'Failed to import products. Please check your CSV and try again.');
    } finally {
      ...
      >
          ...
          {!results && (
            <>
              ...
              <button
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
              >
                {isUploading ? (
                  <>
        ...
        {results && !error && (
          <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded text-green-700">
            Successfully imported {results.successful} products.
          </div>
        )}
      </motion.div>
    </div>
